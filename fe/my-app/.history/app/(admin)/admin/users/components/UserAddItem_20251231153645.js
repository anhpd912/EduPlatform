...
export default function UserAddItem({ onClose, onSuccess }) {
  ...
  const validateStep1 = () => {
    if (!formData.username || !formData.email || !formData.password || !formData.fullName || !formData.phoneNumber || !formData.address || !formData.role) {
    ...
    <>
      <div className={styles.StepHeader}>
        ...
        <p className={styles.StepDescription}>
          {t.parentInfoDescription || "Please provide parent/guardian contact information for the student."}
        </p>
      </div>

      <div className={styles.FormGrid}>
        <div className={styles.FormGroup}>
          <label htmlFor="parentName">{t.parentName || "Parent Name"}</label>
          <input
            type="text"
            id="parentName"
            name="parentName"
        ...
        <div className={styles.FormGroup}>
          <label htmlFor="parentPhone">{t.parentPhone || "Parent Phone"}</label>
          <input
            type="tel"
            id="parentPhone"
            name="parentPhone"
        ...
        {formData.role === "STUDENT" && (
          <div className={styles.StepIndicator}>
            ...
            <div className={`${styles.Step} ${currentStep >= 2 ? styles.StepActive : ""}`}>
              ...
              <span className={styles.StepLabel}>{t.parentInfo || "Parent Info"}</span>
            </div>
          </div>
        )}
