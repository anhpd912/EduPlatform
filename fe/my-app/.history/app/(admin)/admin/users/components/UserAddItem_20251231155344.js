...
export default function UserAddItem({ onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Append studentInfo for Student role
      if (formData.role === "STUDENT") {
        if (parentData.parentName) {
          submitData.append("studentInfo.parentName", parentData.parentName);
        }
        if (parentData.parentPhone) {
          submitData.append("studentInfo.parentPhone", parentData.parentPhone);
        }
      }

      // Append teacherInfo for Teacher role
      if (formData.role === "TEACHER") {
        if (teacherData.expertise) {
          submitData.append("teacherInfo.expertise", teacherData.expertise);
        }
      }
