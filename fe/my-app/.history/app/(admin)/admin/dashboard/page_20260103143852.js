...
export default function AdminDashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [usersRes, studentsRes, classesRes, subjectsRes] =
        await Promise.all([
      ...
      console.log("Parsed - Users:", users.length, "Students:", students.length, "Classes:", classes.length, "Subjects:", subjects.length);
    ...
    if (Array.isArray(users) && users.length > 0) {
      ...
      activities.push({
        ...
        message: `New user registered: ${
          recentUser.fullName || recentUser.username
        }`,
        time: "Recently",
      });
    }
