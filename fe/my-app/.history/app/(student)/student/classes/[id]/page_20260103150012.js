  ...
  return (
    <div className={styles.PageContainer}>
      ...
      <div className={styles.ClassInfoCard}>
        ...
        <Link href={`/student/classes/${classId}/schedule`} className={styles.ScheduleLink}>
            ...
            <div className={styles.InfoSection}>
              ...
              <div className={styles.InfoGrid}>
                ...
                <div className={styles.InfoItem}>
                  ...
                  <span className={styles.InfoValue}>{classInfo.classCode}</span>
                </div>
                <div className={styles.InfoItem}>
                  <span className={styles.InfoLabel}>Ngày bắt đầu</span>
                  <span className={styles.InfoValue}>{formatDate(classInfo.startDate)}</span>
                </div>
                <div className={styles.InfoItem}>
                  <span className={styles.InfoLabel}>Ngày kết thúc</span>
                  <span className={styles.InfoValue}>{formatDate(classInfo.endDate)}</span>
                </div>
                <div className={styles.InfoItem}>
                ...
                {students.map((student, index) => (
                  <div key={student.id || index} className={styles.StudentCard}>
                    ...
                    <div className={styles.StudentInfo}>
                      <h4>{student.student?.fullName || student.fullName || "N/A"}</h4>
                ...
                {subjects.map((subject, index) => (
                  <div key={subject.id || index} className={styles.SubjectCard}>
                    ...
                    <div className={styles.SubjectInfo}>
                      <h4>{subject.subject?.name || subject.subjectName || "N/A"}</h4>
                      ...
                      {subject.subject?.description && (
                        <p className={styles.SubjectDesc}>{subject.subject.description}</p>
                      )}
                    </div>
                  </div>
                ))}
