...
export default function TeacherCalendarPage() {
  ...
  // Map backend dayOfWeek (Java DayOfWeek: 1=Monday, 2=Tuesday, ..., 7=Sunday) to JS day (0=Sunday, 1=Monday, ..., 6=Saturday)
  const dayMapping = {
    1: 1, // Monday
    2: 2, // Tuesday
    3: 3, // Wednesday
  ...
  const generateSchedules = () => {
    ...
    timetables.forEach((timetable) => {
      ...
      if (dayOfWeek !== undefined) {
        // Parse fromDate and toDate from timetable using local timezone
        const fromDate = parseLocalDate(timetable.fromDate);
        const toDate = parseLocalDate(timetable.toDate);
        ...
        console.log("Effective range:", { startDate, endDate });

        // Skip if the timetable is outside the current month
        if (startDate > lastDay || endDate < firstDay) {
          console.log("Skipping - outside current month");
          return;
        }
        ...
        while (date <= endDate) {
          ...
          if (date.getDay() === dayOfWeek) {
            console.log("Match found! Adding schedule for:", date);
            allSchedules.push({
    ...
    console.log("Generated schedules:", allSchedules);
  ...
  const handleEditSchedule = (schedule) => {
    // Find the original timetable data
    const timetable = timetables.find((t) => t.id === schedule.timetableId);
    if (timetable) {
      setEditingSchedule(timetable);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      await TimetableService.delete(deleteConfirm.timetableId);
      // Refresh timetables
        ...
        <div className={styles.DeleteOverlay} onClick={() => setDeleteConfirm(null)}>
          <div className={styles.DeleteModal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.DeleteInfo}>
              <p><strong>{deleteConfirm.className}</strong></p>
              <p>{deleteConfirm.subjectName}</p>
              <p>{deleteConfirm.startTime} - {deleteConfirm.endTime}</p>
            </div>
