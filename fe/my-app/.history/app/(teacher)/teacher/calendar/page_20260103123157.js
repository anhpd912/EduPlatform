...
export default function TeacherCalendarPage() {
  ...
  const generateSchedules = () => {
    ...
    console.log("Generating schedules for:", { year, month, firstDay, lastDay });
    console.log("Timetables:", timetables);

    timetables.forEach((timetable) => {
      const dayOfWeek = dayMapping[timetable.dayOfWeek];
      console.log("Processing timetable:", timetable.id, "dayOfWeek:", timetable.dayOfWeek, "mapped to JS day:", dayOfWeek);

      if (dayOfWeek !== undefined) {
        // Parse fromDate and toDate from timetable (handle timezone properly)
        const fromDate = timetable.fromDate
          ? new Date(timetable.fromDate + "T00:00:00")
          : null;
        const toDate = timetable.toDate 
          ? new Date(timetable.toDate + "T23:59:59") 
          : null;
        ...
        while (date <= endDate) {
          console.log("Checking date:", date, "getDay():", date.getDay(), "expected:", dayOfWeek);
