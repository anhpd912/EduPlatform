...
export default function TeacherClassesPage() {
  ...
  const [isHomeroomForSchedule, setIsHomeroomForSchedule] = useState(true);
  ...
  const handleAddSchedule = (classData, isHomeroom = true) => {
    setSelectedClassForModal(classData);
    setIsHomeroomForSchedule(isHomeroom);
  ...
  const handleAddTeacher = (classData) => {
    setSelectedClassForModal(classData);
  ...
  const handleSaveSubject = async (subjectData) => {
    // Call API to save subject to class
    // subjectData: { classId, subjectId, teacherId }
    await ClassSubjectService.create(subjectData);
    // After saving, refresh classes to get updated data
    await fetchClasses();
  };

  const handleSaveTeacher = async (teacherData) => {
    // Call API to add teacher to class
    // teacherData: { classId, username }
    await ClassTeacherService.create(teacherData);
    // After saving, refresh classes to get updated data
    await fetchClasses();
  };
    ...
    <div className={styles.PageContainer}>
      ...
      {showScheduleModal && selectedClassForModal && (
        <AddScheduleModal
          ...
          isHomeroom={isHomeroomForSchedule}
          teacherId={teacherId}
          onClose={() => {
            setShowScheduleModal(false);
            setSelectedClassForModal(null);
            setIsHomeroomForSchedule(true);
          }}
      ...
      {showTeacherModal && selectedClassForModal && (
        <AddTeacherModal
          classData={selectedClassForModal}
          onClose={() => {
