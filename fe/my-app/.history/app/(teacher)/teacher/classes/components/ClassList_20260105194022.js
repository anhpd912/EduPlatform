...
export default function ClassList({
  ...
  // Compare as strings to avoid UUID format issues
  const homeroomClasses = filteredClasses.filter(
    (cls) => String(cls.homeroomTeacherId) === String(currentUserId)
  );
  const teachingClasses = filteredClasses.filter(
    (cls) => String(cls.homeroomTeacherId) !== String(currentUserId)
  );

  const renderClassGrid = (classList, isHomeroom = false) => (
    <div className={styles.ClassGrid}>
      {classList.map((cls) => (
        <ClassItem
          ...
          onAddSchedule={(classData, isHr) => onAddSchedule && onAddSchedule(classData, isHr)}
        ...
        <>
          ...
          {homeroomClasses.length > 0 && (
            <div className={styles.ClassSection}>
              ...
              {renderClassGrid(homeroomClasses, true)}
            </div>
          )}
          ...
          {teachingClasses.length > 0 && (
            <div className={styles.ClassSection}>
              ...
              {renderClassGrid(teachingClasses, false)}
            </div>
          )}
        </>
      )}
