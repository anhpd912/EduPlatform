...
export default function ClassItem({
  ...
  const teacherCount = classData.classTeachers?.length || 0;
    ...
    <div className={styles.ClassCard}>
      <div className={styles.ClassHeader}>
        <div className={styles.ClassInfo}>
          ...
          {classData.classCode && (
            <span className={styles.ClassCode}>{classData.classCode}</span>
          )}
        ...
        {isHomeroom && (
          <>
            <button
              ...
              onClick={() => onAddTeacher && onAddTeacher(classData)}
              title={t.addTeacher || "Add Teacher"}
            >
              <PersonAddIcon fontSize="small" />
              <span>{t.addTeacher || "Add Teacher"}</span>
            </button>
            <button
              className={styles.QuickActionButton}
              onClick={() => onAddSubject && onAddSubject(classData)}
              title={t.addSubject || "Add Subject"}
            >
              <MenuBookIcon fontSize="small" />
              <span>{t.addSubject || "Add Subject"}</span>
            </button>
          </>
        )}
        <button
          className={styles.QuickActionButton}
          onClick={() => onAddSchedule && onAddSchedule(classData, isHomeroom)}
          title={t.addSchedule || "Add Schedule"}
        >
