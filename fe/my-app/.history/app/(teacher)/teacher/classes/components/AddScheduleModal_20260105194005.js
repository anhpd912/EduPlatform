...
export default function AddScheduleModal({ classData, onClose, onSave, isHomeroom = true, teacherId = null }) {
  const { language } = useLanguage();
  const t = translations[language];

  const [classSubjects, setClassSubjects] = useState([]);
  const [loadingSubjects, setLoadingSubjects] = useState(true);
  const [teacherSubject, setTeacherSubject] = useState(null); // Môn học của giáo viên giảng dạy
  ...
  const fetchClassSubjects = async () => {
    try {
      ...
      // Nếu không phải homeroom teacher, tìm môn mà giáo viên đang dạy
      if (!isHomeroom && teacherId) {
        const mySubject = subjects.find(
          (cs) => String(cs.teacherId) === String(teacherId)
        );
        if (mySubject) {
          setTeacherSubject(mySubject);
          // Tự động set vào form
          setFormData((prev) => ({
            ...prev,
            classSubjectId: `${mySubject.subjectId}|${mySubject.teacherId}`,
          }));
        }
      }
    } catch (err) {
  ...
  // Helper: Map backend dayOfWeek (1=Monday, 2=Tuesday, ..., 7=Sunday) to JS day (0=Sunday, 1=Monday, ..., 6=Saturday)
  const backendDayToJsDay = (backendDay) => {
    if (backendDay === 7) return 0; // Sunday: backend 7 -> JS 0
    return backendDay; // Monday-Saturday: backend 1-6 -> JS 1-6
  };
      ...
      <div className={styles.Modal} onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className={styles.Form}>
          <div className={styles.FormGroup}>
            ...
            {loadingSubjects ? (
            ...
            ) : !isHomeroom && teacherSubject ? (
              // Giáo viên giảng dạy - hiển thị thông tin môn học cố định
              <div className={styles.FixedSubject}>
                <span className={styles.SubjectInfo}>
                  {teacherSubject.subjectName || teacherSubject.subject?.name} -{" "}
                  {teacherSubject.teacherName || teacherSubject.teacher?.fullName}
                </span>
              </div>
            ) : !isHomeroom && !teacherSubject ? (
              <div className={styles.NoSubjects}>
              ...
              // Homeroom teacher - cho phép chọn môn
              <select
                name="classSubjectId"
          ...
          <div className={styles.FormActions}>
            ...
            <button
              ...
              disabled={loading || classSubjects.length === 0 || (!isHomeroom && !teacherSubject)}
            >
