...
export default function NotificationsPage() {
  ...
  const handleAcceptInvitation = async (notification) => {
    try {
      ...
      if (classId && teacherId) {
        ...
        setNotifications((prev) =>
          prev.map((n) =>
            n.id === notification.id ? { ...n, isRead: true, responded: true } : n
          )
        );
        alert(t.invitationAccepted || "Invitation accepted successfully!");
      } else {
        console.error("Missing classId or teacherId in notification data:", data);
        alert("Missing class or teacher information in notification");
      }
    } catch (err) {
  ...
  const handleDeleteNotification = async (notificationId) => {
    if (
      !window.confirm(
        t.confirmDeleteNotification || "Delete this notification?"
      )
    ) {
      return;
    }
  ...
  const getNotificationIcon = (type) => {
    switch (type) {
      ...
      case "WARNING":
        return (
          <Warning className={`${styles.TypeIcon} ${styles.WarningIcon}`} />
        );
      case "SYSTEM":
      default:
