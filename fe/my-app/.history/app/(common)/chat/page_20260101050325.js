...
export default function ChatPage() {
  ...
  const fetchMyClasses = async () => {
    try {
      const response = await ChatService.getMyClasses();
      if (response.data) {
        setMyClasses(response.data);
        // Initialize expanded state for each class
        const expanded = {};
        response.data.forEach((cls) => {
          expanded[cls.id] = false;
        });
    ...
    <div className={styles.chatContainer}>
      ...
      >
        ...
        <div className={styles.searchBox}>
          ...
          <input
            type="text"
            placeholder={activeTab === "chats" ? (t.searchUsers || "Search users...") : (t.searchClasses || "Search classes...")}
            value={searchQuery}
                    ...
                    <div key={cls.id} className={styles.classSection}>
                      ...
                      >
                        ...
                        <div className={styles.classInfo}>
                          ...
                          <span className={styles.classMemberCount}>
                            {cls.studentCount || cls.students?.length || 0} {t.students || "students"}
                            {cls.teacher && ` • 1 ${t.teacher || "teacher"}`}
                          </span>
                        </div>
                        {expandedClasses[cls.id] ? <ExpandLess /> : <ExpandMore />}
                      </div>
                        ...
                        <div className={styles.classMembers}>
                            ...
                            <>
                              ...
                              <div
                                className={`${styles.memberItem} ${
                                  selectedChat?.id === cls.teacher.id ? styles.active : ""
                                }`}
                                onClick={() =>
                                  handleSelectChat({
                                    id: cls.teacher.id,
                              ...
                              >
                                <div className={styles.memberAvatar}>
                                  {cls.teacher.avatar ? (
                                    <img src={cls.teacher.avatar} alt={cls.teacher.username} />
                                  ) : (
                                    <Person />
                                  )}
                                </div>
                                <div className={styles.memberInfo}>
                                  ...
                                  <span className={styles.memberRoleTag}>
                                    {t.teacher || "Teacher"}
                                  </span>
                                </div>
                              </div>
                          ...
                          {cls.students && cls.students.length > 0 && (
                            <>
                              <div className={styles.memberSectionTitle}>
                                <People fontSize="small" />
                                <span>{t.students || "Students"} ({cls.students.length})</span>
                              </div>
                              {cls.students
                                ...
                                .map((student) => (
                                  <div
                                    key={student.id}
                                    className={`${styles.memberItem} ${
                                      selectedChat?.id === student.id ? styles.active : ""
                                    }`}
                                    onClick={() =>
                                      handleSelectChat({
                                        id: student.id,
                                  ...
                                  >
                                    ...
                                    <div className={styles.memberInfo}>
                                      ...
                                      <span className={styles.memberRoleTagStudent}>
                                        {t.student || "Student"}
                                      </span>
                                    </div>
                                  </div>
